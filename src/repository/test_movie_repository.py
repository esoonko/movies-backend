...
def test_movies_get_added_and_retrieved():
	...
	movie_repository.add_movie(movie1)
	movie_repository.add_movie(movie2)

	assert len(movie_repository.get_all_movies()) == 2

	movie_repository.session.rollback()
	movie_repository.session.close()
...
def test_get_movie_doesnt_exist():
	...
	result = movie_repository.find_movie(random_id)
	assert result == None
